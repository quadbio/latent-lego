...
class ArchitectureSurgery:
    ...
    def update_architecture(
        ...
        freeze_encoder_first: bool = True,
        freeze_decoder_first: bool = True,
        freeze_encoder_batchnorm: bool = False,
        freeze_decoder_batchnorm: bool = False,
        ...
        Arguments:
            ...
            freeze_encoder_first: Whether to freeze the first dense layer of the
                encoder.
            freeze_decoder_first: Whether to freeze the first dense layer of the
                decoder.
            freeze_encoder_batchnorm: Whether to freeze batchnorm layers in the
                encoder.
            freeze_decoder_batchnorm: Whether to freeze batchnorm layers in the
                decoder.
        ...
        if freeze_dropout:
            new_model = self.clone_model(
                model, conditional=add_condition, dropout_rate=0
            )
        else:
        ...
        if old_conditional:
            if model._conditional_encoder():
                old_cond_shape = (
                    model.encoder.layers[0]._saved_model_inputs_spec[1].shape[-1]
                )
            else:
                old_cond_shape = (
                    model.decoder.layers[0]._saved_model_inputs_spec[1].shape[-1]
                )
        ...
        if unfrozen:
            ...
            new_model = self.freeze_layers(
                ...
                freeze_decoder_first=freeze_decoder_first,
                freeze_encoder_batchnorm=freeze_encoder_batchnorm,
                freeze_decoder_batchnorm=freeze_decoder_batchnorm,
    ...
    def clone_model(self, model, **kwargs):
        ...
        encoder = model.encoder.__class__(
            **encoder_params["args"], **encoder_params["kwargs"]
        )
        decoder = model.decoder.__class__(
            **decoder_params["args"], **decoder_params["kwargs"]
        )
    ...
    #     freeze_decoder_first=True,
    #     freeze_encoder_batchnorm=False,
    #     freeze_decoder_batchnorm=False,
    #     freeze_weights=None,
    # ):
    #     """Freeze required layers of a model."""
    #     for layer in model.layers:
    #         if freeze_weights and layer.name in freeze_weights:
    #             layer.trainable = False
    #         if freeze_dropout and isinstance(layer, keras.layers.Dropout):
    #             layer.trainable = False
    #         if freeze_encoder_first and isinstance(layer, keras.layers.Dense):
    #             layer.trainable = False
    #         if freeze_decoder_first and isinstance(layer, keras.layers.Dense):
    #             layer.trainable = False
    #         if freeze_encoder_batchnorm and isinstance(
    #             layer, keras.layers.BatchNormalization
    #         ):
    #             layer.trainable = False
    #         if freeze_decoder_batchnorm and isinstance(
    #             layer, keras.layers.BatchNormalization
    #         ):
    #             layer.trainable = False
    #     return model
