...
def aggregate(ary, groups=None, fun=np.mean, axis=0):
    ...
    if groups is None:
        ...
        split_ary = np.split(
            ary, np.unique(groups, return_index=True)[1][1:], axis=axis
        )
...
def total_correlation(z, mu, scale):
    ...
    log_qz_prob = log_density_gaussian(
        tf.expand_dims(z, 1), tf.expand_dims(mu, 0), tf.expand_dims(scale, 0)
    )
...
KERNELS = {"ms_rbf": ms_rbf_kernel, "rbf": rbf_kernel, "rq": rq_kernel}
...
def persistent_homology(matrix):
    return tf.numpy_function(_persistent_homology, [matrix], tf.int64)
...
def _gromov_wasserstein_distance(x, y):
    ...
    gw_dist = ot.gromov.gromov_wasserstein2(x, y, x_p, y_q, loss_fun="kl_loss")
...
def _entropic_gromov_wasserstein_distance(x, y):
    x_p = ot.unif(x.shape[0])
    y_q = ot.unif(y.shape[0])
    gw_dist = ot.gromov.entropic_gromov_wasserstein2(x, y, x_p, y_q, loss_fun="kl_loss")
    return np.array(gw_dist, dtype=np.float32)


def gromov_wasserstein_distance(x, y):
    return tf.numpy_function(_gromov_wasserstein_distance, [x, y], tf.float32)


def entropic_gromov_wasserstein_distance(x, y):
    return tf.numpy_function(_entropic_gromov_wasserstein_distance, [x, y], tf.float32)
